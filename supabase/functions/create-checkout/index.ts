...
serve(async (req) => {
  ...
  try {
    ...
    let unitAmount;
    if (priceType === 'annual') {
      unitAmount = vehicleCount * 216000; // $2160/year per vehicle
    } else {
      unitAmount = vehicleCount * 20000; // $200/month per vehicle
    }
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      subscription_data: {
        metadata: {
          ...
          vehicle_id: vehicles && vehicles.length > 0 ? 'multiple' : 'pending'
        }
      },
