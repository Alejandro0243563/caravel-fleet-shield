...
const Success = () => {
  ...
  // Update vehicle status and subscription after successful payment
  useEffect(() => {
    window.scrollTo(0, 0);
    ...
    const updateAfterPayment = async () => {
      ...
      if (sessionId) {
        try {
          // Call stripe-post-checkout-upsert to update subscriptions
          await supabase.functions.invoke('stripe-post-checkout-upsert', {
            body: { session_id: sessionId },
          });
          console.log('Subscription updated successfully');
        } catch (e) {
          console.error('post-checkout-upsert failed', e);
        }
      }
  ...
  return (
    <>
      <SEO 
        title="Pago Exitoso - CARAVEL México"
        description="Tu suscripción CARAVEL ha sido procesada exitosamente. Protección contra multas activada."
        canonical="https://caravel.com/success"
        noIndex={true}
      />
